...
export const Drivers = () => {
  ...
  const searcher = (e) => {
    setSearch(e.target.value); // guardamos el texto tal cual
  };

  // filtrar los pilotos
  const filteredDrivers = drivers
    ? drivers.filter((data) => {
        const searchClean = search.trim().toLowerCase(); // limpiamos solo acá
        return (
          data.full_name.toLowerCase().includes(searchClean) ||
          data.team_name.toLowerCase().includes(searchClean) ||
          data.driver_number.toString().includes(searchClean)
        );
      })
    : [];
  ...
  return (
    <>
      ...
      <div className="flex w-full justify-between">
        ...
        />
        <p className="text-gray-600 text-xs self-center pr-1 text-end">
          Selecione un piloto para más información
        </p>
      </div>
